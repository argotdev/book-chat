...
export class PDFEmbeddingSystem {
      ...
      for (const [pageNum, pageText] of Object.entries(pdfText)) {
        ...
        for (let i = 0; i < chunks.length; i++) {
          ...
          if (vectors.length >= 100) {
            await this.index.namespace(pdfName).upsert(vectors);
  ...
  async querySimilar(query, topK = 3, filterDict = null, pdfName) {
    try {
      const queryEmbedding = await this.createEmbedding(query);

      const results = await this.index.namespace(pdfName).query({
